...
export const processAndEmbedIndividualMaterial = schemaTask({
  ...
  run: async (payload: ProcessAndEmbedIndividualPayloadType) => {
    ...
    await logger.info(
      "🟢 [STANDALONE] Starting individual material processing",
      {
        materialId,
        filePath,
        contentType,
    ...
    try {
      ...
      await logger.info(
        `File downloaded successfully: ${filePath}, size: ${buffer.length} bytes`
      );
      ...
      await logger.info(
        "✅ [STANDALONE] Individual material processing complete",
        {
          materialId,
      ...
      const errorMessage =
        error instanceof Error ? error.message : "An unknown error occurred";
      await logger.error(
        "❌ [STANDALONE] Individual material processing failed",
        {
          materialId,
          filePath,
          contentType,
