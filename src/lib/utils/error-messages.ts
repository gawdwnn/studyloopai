...
export interface ErrorClassification {
  type: 'validation' | 'constraint' | 'network' | 'file' | 'auth' | 'quota' | 'server' | 'unknown';
  userMessage: string;
  actionable: boolean;
  retryable: boolean;
}
...
export function classifyError(error: Error | string): ErrorClassification {
  ...
  if (lowerMessage.includes('unique constraint') || 
    ...
    return {
      type: 'constraint',
      userMessage: "This week already has materials uploaded. Please select a different week.",
      actionable: true,
      retryable: false
    };
  }
  ...
  // File-related errors
  if (lowerMessage.includes('file too large') || lowerMessage.includes('exceeds maximum size')) {
    return {
      type: 'file',
      userMessage: "File size exceeds the 10MB limit. Please choose a smaller file.",
      actionable: true,
      retryable: false
    };
  }
...
export const UPLOAD_ERROR_MESSAGES = {
  WEEK_HAS_MATERIALS: "This week already has materials uploaded. Please select a different week.",
  FILE_TOO_LARGE: "File size exceeds the 10MB limit. Please choose a smaller file.",
  ...
  PROCESSING_FAILED: "Files uploaded successfully but processing failed to start. We'll retry automatically.",
...
export function getUploadErrorMessage(error: Error | string): string {
  ...
  if (lowerMessage.includes('unique constraint') || lowerMessage.includes('already exists')) {
  ...
  if (lowerMessage.includes('invalid file type') || lowerMessage.includes('only pdf')) {
