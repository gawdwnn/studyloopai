...
**AI-powered adaptive learning platform that transforms course materials into personalized study experiences**
...
- pgvector
- Drizzle ORM
- Supabase (Auth, Database, Storage, VectorDB, RLS)
  - **Development:** Local Supabase CLI
...
# 3. Setup local database (development)
supabase start                      # Start local Supabase
bun run db:migrate                  # Apply migrations
./scripts/apply-local-policies.sh   # Apply RLS policies

# For production deployment, see Supabase Operations below
...
```bash
supabase start                          # Start local Supabase
supabase stop --backup                  # Stop with data persistence
supabase status                         # Check local services
./scripts/apply-local-policies.sh       # Apply RLS policies locally
```
...
bun scripts/supabase-manager.ts switch --env prod  # Switch to production
bun scripts/supabase-manager.ts policies --apply   # Apply policies to prod
bun scripts/supabase-manager.ts status             # Check production status
```
