...
const MANAGED_TABLES = [
  "users", "user_plans", "courses", "course_weeks", "course_materials",
  "document_chunks", "cuecards", "multiple_choice_questions", "open_questions",
  "summaries", "golden_notes", "concept_maps", "own_notes", "generation_configs",
  "user_prompt_templates", "institutions", "user_progress",
] as const;
...
const getCurrentProject = withRetry(async (): Promise<ProjectKey | null> => {
  try {
    ...
    const linkedProject = projects.find((p: { linked: boolean; id: string }) => p.linked);
...
const linkProject = async (projectKey: ProjectKey, dbPassword: string): Promise<void> => {
  const project = PROJECTS[projectKey];
  if (!project.ref) {
...
const getPolicyFiles = async (): Promise<string[]> => {
  ...
  const files = readdirSync(POLICIES_DIR)
    .filter(file => file.endsWith(".sql") && /^[a-zA-Z0-9_-]+\.sql$/.test(file))
    .sort();
...
const applyPolicyFile = async (client: DatabaseClient, filename: string): Promise<{ status: 'success' | 'warning' | 'error', message?: string }> => {
    ...
    if (error instanceof Error && 'code' in error) {
      ...
      switch (pgError.code) {
        case '42710': // duplicate_object - policy already exists
          process.stdout.write(chalk.yellow(`⚠️  Policy in ${filename} already exists - skipping\n`));
          return { status: 'warning', message: 'Policy already exists' };
          
        case '42P01': // undefined_table - table does not exist  
          process.stdout.write(chalk.yellow(`⚠️  Table referenced in ${filename} does not exist - skipping\n`));
        ...
        default:
          ...
          logger.error(`Unknown PostgreSQL error code ${pgError.code} for ${filename}: ${errorMessage}`);
      }
    }
...
const confirmProductionOperation = async (operation: string): Promise<boolean> => {
  process.stdout.write(chalk.red.bold("\n🚨 PRODUCTION ENVIRONMENT\n"));
  process.stdout.write(chalk.yellow(`You are about to ${operation} on PRODUCTION.\n`));
  process.stdout.write(chalk.gray("This operation may affect live users and data.\n\n"));
  ...
  if (confirmation !== "PRODUCTION") {
    process.stdout.write(chalk.yellow("Operation cancelled - confirmation text did not match\n"));
    return false;
  }
...
const executeOperation = async (projectKey: ProjectKey, operation: string): Promise<void> => {
  ...
  switch (operation) {
      ...
      await withDatabase(projectKey, async (client) => {
        ...
        for (const [table, tablePolicies] of Object.entries(policyByTable)) {
          ...
          for (const policy of tablePolicies) {
            process.stdout.write(chalk.gray(`     - ${policy.policyname} (${policy.cmd})\n`));
          }
        }
      });
      break;
    ...
    case "list-policies": {
      ...
      for (const file of files) {
        ...
        process.stdout.write(chalk.gray(`     • ${parsed.policies.length} policies\n`));
        process.stdout.write(chalk.gray(`     • ${parsed.tables.length} tables: ${parsed.tables.join(", ")}\n`));
      }
      break;
    }
    ...
    case "validate-rls": {
      await withDatabase(projectKey, async (client) => {
        ...
        const missingRLS = MANAGED_TABLES.filter(table =>
          !rlsStatus.some(status => status.tablename === table && status.rls_enabled)
        );
    ...
    case "logs": {
      ...
      for (const service of services) {
        try {
          process.stdout.write(chalk.cyan(`--- ${service.toUpperCase()} Service ---\n`));
          
          // This would use MCP Supabase tools in a real implementation
          // For now, show a placeholder
          process.stdout.write(chalk.gray(`Logs for ${service} service would appear here\n`));
          process.stdout.write(chalk.gray("(Requires MCP Supabase integration)\n\n"));
        } catch (error) {
          process.stdout.write(chalk.red(`Failed to fetch ${service} logs: ${error}\n`));
        }
      }
      break;
    }
      ...
      const tasks = new Listr([
        {
          ...
          task: () => runCommand("bun", ["run", "db:migrate"], "Applying migrations"),
        },
      ]);
      await tasks.run();
      break;
    ...
    case "apply-policy": {
      ...
      await withDatabase(projectKey, async (client) => {
        ...
        if (result.status === 'success') {
          process.stdout.write(chalk.green(`✅ Policy file ${selectedFile} applied successfully\n`));
    ...
    case "apply-policies": {
      ...
      const tasks = new Listr([
        {
          ...
          task: () => withDatabase(projectKey, async (client) => {
      ...
      // Summary output
      if (errorCount > 0) {
        process.stdout.write(chalk.red(`❌ ${errorCount} policies failed, ${successCount} succeeded, ${warningCount} skipped\n`));
      } else if (warningCount > 0) {
        process.stdout.write(chalk.yellow(`⚠️  ${successCount} policies applied, ${warningCount} already existed (skipped)\n`));
      } else {
        process.stdout.write(chalk.green(`✅ All ${successCount} policy files applied successfully\n`));
      }
      break;
    }
      ...
      const tasks = new Listr([
        {
          ...
          task: () => runCommand("bun", ["run", "db:generate"], "Generating migrations"),
        },
        {
          title: "Applying migrations", 
          task: () => runCommand("bun", ["run", "db:migrate"], "Applying migrations"),
            ...
            await withDatabase(projectKey, async (client) => {
              await pMap(files, async (file) => {
                ...
                if (result.status === 'error') {
                  logger.error(`Setup failed: could not apply ${file}: ${result.message}`);
                }
                return result;
              }, { concurrency: 1 });
            });
  ...
  .action(async () => {
    try {
      ...
      if (current) {
        ...
        process.stdout.write(chalk.gray("Use 'switch' command to link a project\n"));
      }
  ...
  .action(async (options) => {
    try {
      ...
      if (current === targetProject) {
        process.stdout.write(chalk.green(`✓ Already connected to ${PROJECTS[targetProject].name}\n`));
        return;
      }
      ...
      await tasks.run();
      process.stdout.write(chalk.green(`✅ Successfully linked to ${PROJECTS[targetProject].name}\n`));
    } catch (error) {
...
program
  ...
  .action(async () => {
    try {
      ...
      await tasks.run();
      process.stdout.write(chalk.green(`✅ Successfully unlinked from ${project.color(project.name)}\n`));
    } catch (error) {
...
program
  ...
  .action(async () => {
    try {
      ...
      if (current !== "dev") {
        process.stdout.write(chalk.blue("🔧 Switching to Development Environment\n"));
        ...
        process.stdout.write(chalk.green("✅ Linked to development environment\n"));
      }
...
program
  ...
  .action(async (options) => {
    try {
      ...
      if (current !== "prod") {
        if (!options.force) {
          const confirmed = await confirmProductionOperation("switch to production");
          if (!confirmed) return;
        }
        ...
        process.stdout.write(chalk.red("✅ Linked to production environment\n"));
      }
...
program.action(async () => {
  try {
    process.stdout.write(chalk.blue.bold("🚀 Supabase Environment Manager\n\n"));
    
    const current = await getCurrentProject();
    if (current) {
      const project = PROJECTS[current];
      process.stdout.write(`Currently linked to: ${project.color(project.name)}\n\n`);
    }
    ...
    switch (mainOperation) {
      ...
      case "help":
        ...
        break;
      default:
        process.stdout.write(chalk.red(`Unknown operation: ${mainOperation}\n`));
    }
  } catch (error) {
